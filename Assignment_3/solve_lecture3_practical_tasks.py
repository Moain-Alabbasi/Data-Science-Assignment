...
تطبيق كامل يغطي خطوات التحليل الاستكشافي للبيانات (EDA)
 على ملف sales_data.csv المرفق،
  مع التركيز على التصور البياني باستخدام
Matplotlib وSeaborn. يتضمن التطبيق:

تحميل البيانات من ملف sales_data.csv واكتشافها.
معالجة البيانات المفقودة.
إنشاء تصورات بيانية أساسية:
Line Plot (تطور المبيعات عبر الزمن)
Histogram (توزيع المبيعات)
Bar Chart (المبيعات حسب المنطقة)
Scatter Plot (العلاقة بين عمر العميل والمبيعات)
تحليل متقدم:
تحديد التوزيع (طبيعي أو منحرف).
Box Plot للمبيعات حسب المنتج.
Heatmap لتحليل الارتباطات.
تحليل ANOVA.
الحل:
